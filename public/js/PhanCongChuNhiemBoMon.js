(function () {
  function initPhanCong() {
    ...
    function displayAssignments(data) {
      console.log('[FRONTEND DEBUG] Tổng assignments nhận được:', data.length);
      console.log('[FRONTEND DEBUG] Mẫu dữ liệu:', data.slice(0, 3));
      ...
      assignmentTableTbody.innerHTML = data.map((a, i) => {
        const maGVBM = a.MaGVBM ? String(a.MaGVBM).trim() : '(rỗng)';
        const tenGV = a.TenGiaoVien || 'Không xác định';

        // DEBUG TỪNG DÒNG
        console.log(`[RENDER DEBUG] Dòng ${i+1}: MaGVBM='${maGVBM}' | Tên GV='${tenGV}' | Môn='${a.TenMonHoc}' | Lớp='${a.MaLop}'`);

        return `
          <tr data-magvbm="${escapeHtml(maGVBM)}"
              data-malop="${escapeHtml(a.MaLop || '')}"
              data-tenmon="${escapeHtml(a.TenMonHoc || '')}"
              data-namhoc="${escapeHtml(a.NamHoc || '')}"
              data-hocky="${escapeHtml(a.HocKy || '')}">
            <td>${i + 1}</td>
            ...
            <td>
              ${maGVBM !== '(rỗng)' ?
                '<button class="delete-bomon-assign" style="background:#d9534f;color:white;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:12px;">Xóa</button>' :
      ...
      assignmentTableTbody.querySelectorAll('.delete-bomon-assign').forEach(btn => {
        btn.onclick = async (e) => {
          e.preventDefault();
          const tr = e.target.closest('tr');
          const MaGVBM = tr.dataset.magvbm?.trim();

          console.log('[CLICK DEBUG] MaGVBM khi click:', MaGVBM);

          if (!MaGVBM || MaGVBM === '(rỗng)') {
            alert('Lỗi: Thiếu mã phân công. Tải lại trang.');
            return;
          }

          const TenMonHoc = tr.dataset.tenmon;
          const MaLop = tr.dataset.malop;
          const NamHoc = tr.dataset.namhoc;
          const KyHoc = tr.dataset.hocky;
          const gvName = tr.querySelector('td:nth-child(4)').textContent.trim();

          if (!confirm(
            `XÁC NHẬN XÓA\n\n` +
            `Môn: ${TenMonHoc}\n` +
            `Lớp: ${MaLop}\n` +
            `Giáo viên: ${gvName} (${MaGVBM})\n` +
            `Năm: ${NamHoc} - HK${KyHoc}\n\n` +
            `Lịch dạy cũng sẽ bị xóa!`
          )) return;
          ...
          try {
            const res = await fetch('/api/phancongchunhiembomon/delete-bomon-assign', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ MaGVBM, MaLop, TenMonHoc, NamHoc, KyHoc })
            });

            const result = await res.json();
            if (!result.success) throw new Error(result.message || 'Xóa thất bại');

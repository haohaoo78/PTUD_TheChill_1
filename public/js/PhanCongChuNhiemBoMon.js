    ...
    (function () {
      function initPhanCong() {
        ...
        // ===== PHÂN CÔNG BỘ MÔN - PHIÊN BẢN HOÀN CHỈNH ĐÃ SỬA =====
        assignBomonBtn?.addEventListener('click', async () => {
          ...
          // Kiểm tra đầu vào - luôn hiện thông báo
          ...
          // Hiển thị loading (sử dụng alert đơn giản hoặc bỏ, ở đây bỏ để tránh blocking)
          try {
            // 1. Kiểm tra định mức
            ...
            if (!check.canAssign) {
              alert(`Không thể phân công: Vượt định mức: ${check.currentLoad} + ${check.addedLoad} > ${check.MAX_LOAD} tiết`);
              return;
            }
            ...
            // Luôn hiện kết quả
            alert(result.success ? 'Thành công: ' + (result.message || 'Phân công hoàn tất') : 'Lỗi: ' + (result.message || 'Có lỗi xảy ra'));
            ...
            if (result.success) {
              ...
              // Tự động lọc danh sách phân công theo môn + giáo viên vừa thêm
              if (filterMonSelect) {
